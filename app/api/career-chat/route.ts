...
5. 当用户问"我的专业能做什么"或类似问题时，优先参考提供的【求职知识库】里的专业→岗位对照，里面有真实数据；不要只凭你的预训练知识。
6. 纯文本对话,不要任何 Markdown 标记(不要 **、#、---、- 或 1. 列表)。要分点就用 ① ② ③ 或「」,像微信里自然聊天那样。`;
...
export const POST = apiPost({ credit: "chat", requireKey: "DEEPSEEK_API_KEY" }, async (body) => {
  ...
  const hardNo = Array.isArray(body.hardNo) ? (body.hardNo as string[]).filter((x) => typeof x === "string").join("、") : "";
  const context =
    `【简历原文】\n${clip(String(body.resumeText ?? "").trim() || "（求职者未提供简历正文）", 4000)}\n\n` +
    `【求职意愿】\n目标方向：${String(body.targetDirection ?? "").trim() || "（未填，可在对话里引导他说清）"}\n绝不接受：${hardNo || "（未填）"}`;

  // knowledge 注入
  const lastUser = history.filter((m) => m.role === "user").pop();
  let knowledgeBlock = "";
  if (lastUser) {
    const paragraphs = search(lastUser.content, 3);
    if (paragraphs.length > 0) knowledgeBlock += `\n\n【求职知识库·相关方法论】\n${formatForPrompt(paragraphs)}`;
    let majorHit = matchMajor(lastUser.content);
    if (!majorHit) majorHit = matchMajor(String(body.resumeText ?? ""));
    if (majorHit) knowledgeBlock += `\n\n【求职知识库·专业岗位对照】\n专业「${majorHit.major}」的适合方向：\n${majorHit.rawBlock}`;
  }

  const payload = {
    model,
    temperature: 0.6,
    messages: [{ role: "system", content: `${SYSTEM}\n\n${context}${knowledgeBlock}` }, ...history.map((m) => ({ role: m.role, content: m.content }))]
  };
