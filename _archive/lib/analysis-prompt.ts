...
注意：score（输出 0）、matchedKeywords（输出 []）、missingKeywords（输出 []）、history（输出单个占位条目即可）均由后端覆盖或派生生成，你只需输出 schema 占位值，不要展开解释这些字段。
...
4. segments.original 只负责提供可逐字匹配的连续短片段：必须逐字复制最终 resumeDisplayText 中的一段连续文本，保持原语言，不得翻译、改写、概括、补字、纠错或润色。
...
13. annotations.original 只负责提供可逐字匹配的连续短片段：必须逐字引用最终 resumeDisplayText 中的一段连续文本，优先选择 15-60 字的短片段。不得翻译、改写、概括、补字、纠错、润色或拼接不连续片段。每条 annotation 只围绕原文中的”一件事”分析。
...
20. annotations.section 无法可靠判断时可省略。不要输出 annotations.startIndex / annotations.endIndex；索引由服务端基于 annotations.original 在 resumeDisplayText 中定位生成，模型不要尝试计算。
...
定位关系：annotations.startIndex / annotations.endIndex 由服务端根据 original 自动定位，模型不要计算或输出。
...
6. 不要输出 annotations.startIndex / annotations.endIndex；索引由服务端根据 original 自动定位。
7. rewriteExample 不得加入原文没有的数字、百分比、增长率、用户数、准确率、排名、营收或获奖结果。`;
}
...
4. 再基于整理后的简历原文生成 annotations 和 segments，确保 original 都是可逐字定位的连续短片段；不要计算或输出 startIndex / endIndex。
