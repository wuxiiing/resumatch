...
只输出合法 JSON object，不要 markdown、不要代码块、不要前后缀。结构：
{
  ...
  "interviewFocus": [
    {
      "area": "考察领域（如「算法/技术面」「项目经历」「行为面」「业务理解」「文化匹配」等）",
      "weight": 40,
      "reason": "JD 里的依据——为什么这块是重点、这家公司在这块会考多深"
    }
  ],
...
- interviewFocus 给 3-5 项：根据 JD 真身（jdAnalysis）推测这家公司面试会重点考什么、各占多少比重。weight 加起来应在 90-100 左右，按重要性排序。reason 要引用 JD 里的具体信号（写了什么要求/技术栈/文化关键词），不能泛泛说"这是常规考察"。
